<template>
  <div>
   ...
   <div>
      ...
      <h1 class="back" style=" visibility:hidden">정기예금</h1>
    <div class="search-box">
      ...
      <hr>
      <h6 style="margin-top:30px; font-family: GangwonEdu_OTFBoldA; font-size:20px;" >은행을 선택하세요</h6>
      <div>
        <select   style="margin-top:10px; font-family: SUITE-Regular; font-size: 16px;" id="bank" v-model="choose_bank" @change="change">
          <option value="" style="color:gray; , sans-serif; ">전체은행</option>
          <option v-for="bank in aboutChange" :value="bank" :key="bank">{{ bank }}</option>
          
        </select>
      </div><br>
      <h6 style="font-family: GangwonEdu_OTFBoldA; font-size:20px;" >예치기간</h6>
      <div>
    ...
    <div class="show-box" style="border-radius: 10px; ">
      ...
      <table>
        ...
        <tbody>
          <tr class="fixed-height"  style="height:74px;margin-top:0px;margin-bottom:0px;" @click="$router.push(`/finance/deposit/detail/${deposit.fin_prdt_cd}`)" v-for="deposit in pagedDeposits" :key="deposit.fin_prdt_cd">
            <td v-bind:style="{ height: '74px' }">{{ deposit.dcls_month }}</td>
            <td v-bind:style="{ height: '74px' }">{{ deposit.kor_co_nm }}</td>
            <td v-bind:style="{ height: '74px' }" >{{ deposit.fin_prdt_nm }}</td>
            <td v-for="period in periods" :key="period" v-bind:style="{ height: '74px', ...getCellColor(deposit, period) }">{{ getInterestRate(deposit, period) }}</td>
...
export default {
  ...
  mounted() {
    ...
    this.$axios
      .get('http://127.0.0.1:8000/api/v2/deposit-products/', {
            headers: {
  ...
  methods: {
    ...
    // if (this.choose_period && this.choose_period !== '전체기간') {
    //   filtered = filtered.filter(deposit => deposit.save_trm === parseInt(this.choose_period));
    //   }
    ...
    getInterestRate(deposit, period) {
      if (!deposit.depositoptions_set) {
        return '-';
      }
      const options = deposit.depositoptions_set;
      const option = options.find(option => option.save_trm === parseInt(period))

      return option ? `${option.intr_rate}%` : '-'; // 일치하는 객체가 없을 경우 -을 출력
    },
    prevPage() {
...
tr td, tr th {
  height: 82px; /* 원하시는 높이로 변경하세요 */
  overflow: hidden; /* 이 속성은 요소의 높이가 height를 초과하는 경우 내용을 숨깁니다. */
}
